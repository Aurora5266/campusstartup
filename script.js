// Campus Startup Marketplace Application - Functional Approach
...
function renderStartupShowcase() {
  const content = document.getElementById('main-content');
  content.innerHTML = `
    ...
    <div class="grid" id="startupsGrid">
      ${renderStartupCards(true)}
    </div>
  `;
}

function renderMentorConnect() {
  const content = document.getElementById('main-content');
  content.innerHTML = `
    <div style="text-align: center; margin-bottom: 3rem;">
      ...
      <div class="card">
        ...
        <div style="margin-top: 1rem;">
          ${['Technology', 'Education', 'Food & Dining', 'Health & Fitness', 'Sustainability']
            .map(category => {
              const count = startups.filter(s => s.category === category).length;
              return `
                <div style="display: flex; justify-content: space-between; padding: 0.5rem 0; border-bottom: 1px solid var(--border-light);">
                  <span>${category}</span>
...
function renderStartupCards(detailed = false) {
...
function purchaseProduct(startupId) {
  const startup = startups.find(s => s.id === startupId);
  if (!startup) return;

  if (appState.walletBalance >= startup.price) {
    appState.walletBalance -= startup.price;
    ...
    updateWalletDisplay();
    showMessage(`Successfully purchased ${startup.product} for $${startup.price}!`, 'success');
  } else {
    showMessage('Insufficient funds. Please add money to your wallet.', 'error');
  }
}
  ...
  modal.innerHTML = `
    <div class="modal-content">
      ...
      <div class="wallet-balance">
        ...
        <div class="balance-amount" id="modalBalance">$${appState.walletBalance.toFixed(2)}</div>
      </div>
...
function updateWalletModal() {
  ...
  if (modalBalance) {
    modalBalance.textContent = `$${appState.walletBalance.toFixed(2)}`;
  }
  
  if (transactionList) {
    transactionList.innerHTML = renderTransactions();
  }
}

function renderTransactions() {
  return appState.transactions.slice(0, 5).map(transaction => `
...
function addFunds() {
  ...
  updateWalletDisplay();
  updateWalletModal();
  showMessage('Successfully added $50.00 to your wallet!', 'success');
}

function updateWalletDisplay() {
  const walletBtn = document.querySelector('.wallet-btn');
  if (walletBtn) {
    walletBtn.textContent = `Wallet: $${appState.walletBalance.toFixed(2)}`;
  }
}
...
function initializeApp() {
  ...
  // Setup event listeners and render initial content
  setupEventListeners();
  renderMarketplace();
  updateWalletDisplay();
}
